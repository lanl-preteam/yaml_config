...
def is_sib_key(sib_name):
    """Create a post-validator function that makes sure the value of this item is a
    key in the sibling dictionary 'sib_name'. Raises a ValueError if not.
    ...
    def is_sib_key_val(siblings, value):
        if value not in siblings[sib_name].keys():
            raise ValueError("Must be a key of {}, but got {}".format(sib_name, value))
