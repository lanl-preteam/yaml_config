...
class KeyedElem(_DictElem):
    ...
    def normalize(self, values):
        ...
        result in an error.
        :param dict values: The dict of values to normalize.
        ...
        for key, value in values.items():
            ...
            ndict[key] = elem.normalize(value)
    ...
    def validate(self, values, partial=False):
        ...
        for key, elem in self.config_elems.items():
            if isinstance(elem, DerivedElem):
                ...
                values[key] = self.config_elems[key].validate(
                    values.get(key),
                    partial=partial,
                )
        ...
        for elem in derived_elements:
            values[elem.name] = self._find_resolver(elem)(values)

        # Run custom post validation against each key, if given.
        for key, elem in self.config_elems.items():
            values[key] = self._run_post_validator(elem, values,
                                                   values[key])
